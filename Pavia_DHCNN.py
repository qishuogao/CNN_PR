...
from de_conv import de_convolution
...
def train():
    ...
    weights_d4 = tf.Variable(tf.zeros([3, 3, 108, 216]) + 0.001, name="weights_d4")
    offset4 = tf.nn.conv2d(conv4, weights_d4, strides=[1, 1, 1, 1], padding='SAME')
    offset_img4 = deformable_convolution(conv4, offset4)

    # conv_pool
    weights_pool = tf.get_variable("weigts_pool", shape=[2, 2, 108, 108],
                                   dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer(False))
    conv_pool = tf.nn.conv2d(offset_img4, weights_pool, strides=[1, 2, 2, 1], padding='SAME')
    conv_pool = tf.layers.batch_normalization(conv_pool, training=training_flag)
    conv_pool = tf.nn.relu(conv_pool)

    # conv5
    weights5 = tf.get_variable("weigts5", shape=[3, 3, 108, 128],
                               dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer(False))
    conv5 = tf.nn.conv2d(conv_pool, weights5, strides=[1, 1, 1, 1], padding='SAME')
